    ...
    <div className="gallery">
      ...
      <div className="gallery-items">
        {currentItems.map(dog => (
          <div key={dog.id} className="gallery-item" onClick={() => showPetDetails(dog)}>
            <img src={dog.image} alt={dog.name} />
            <div className="overlay">
              <h2 className="pettype">{dog.name}</h2>
              <p className="origin"><strong>Origin:</strong> {dog.origin}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {isPopupVisible && selectedPet && (
        <div className="popup">
          <div className="popup-content" style={{ width: "80%", maxWidth: "800px" }}>
            ...
            <p><strong>Breed Group:</strong> {selectedPet.breed_group}</p>
            <p><strong>Size:</strong> {selectedPet.size}</p>
            <p><strong>Lifespan:</strong> {selectedPet.lifespan}</p>
